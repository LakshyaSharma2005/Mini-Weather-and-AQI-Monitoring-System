...
// --- Dashboard (Fully Formatted HTML, CSS, JS) ---
  ...
  <style>
    ...
    * { box-sizing: border-box; font-family: 'Inter', sans-serif; }
    ...
    @media (max-width: 768px) { 
      .stat-card { grid-column: span 12; height: auto; } 
...
<body>
  <div class="wrapper">
    <div class="header">
      <div class="title">
        <h1>Mini Weather & AQI Monitoring System</h1>
        <div class="sub">IoT Environment Station • <span id="ip">...</span></div>
      </div>
      <div class="controls">
    ...
    <div class="grid">
      <div class="stat-card" style="border-top:4px solid var(--cyan)">
        <div class="stat-label">Temperature</div>
        <div class="stat-val" style="color:var(--cyan)"><span id="temp">--</span> °C</div>
      </div>
      <div class="stat-card" style="border-top:4px solid var(--pink)">
        <div class="stat-label">Humidity</div>
        <div class="stat-val" style="color:var(--pink)"><span id="hum">--</span> %</div>
      </div>
      <div class="stat-card" style="border-top:4px solid var(--orange)">
        <div class="stat-label">Air Quality</div>
        <div class="stat-val" style="color:var(--orange)"><span id="mq2">--</span></div>
        <div>
          <input type="range" id="thr" min="200" max="3000" value="700">
          <div style="font-size:11px; color:var(--muted); margin-top:5px">
            Threshold: <span id="thrText">700</span> 
            <button id="btnSet" class="btn" style="padding:2px 6px; font-size:10px; margin-left:5px">SET</button>
          </div>
        </div>
      </div>
      <div class="big-card">
        <div class="stat-label">Temperature Trend</div>
        <div class="chart-container"><canvas id="chartTemp"></canvas></div>
      </div>
      <div class="big-card">
        <div class="stat-label">Humidity Trend</div>
        <div class="chart-container"><canvas id="chartHum"></canvas></div>
      </div>
      <div class="big-card">
        <div class="stat-label">Gas Level Trend</div>
  ...
  <script>
    const MAX_PTS = 60; 
    let history = { ts:[], t:[], h:[], m:[] };
    
    const r = document.getElementById('thr'); 
    r.addEventListener('input', () => document.getElementById('thrText').innerText = r.value);
    ...
    document.getElementById('btnClear').onclick = async () => { 
      if(confirm("Delete CSV?")) { 
        await fetch('/clear'); 
        document.getElementById('log').innerHTML="<div>Cleared</div>"; 
      }
    };
    ...
    function grad(ctx, c) { 
      ...
      g.addColorStop(0, c.replace(')',',0.5)').replace('rgb','rgba')); 
      g.addColorStop(1, c.replace(')',',0)').replace('rgb','rgba')); 
      return g; 
    }
    
    function mkChart(id, c, l) { 
      const ctx = document.getElementById(id).getContext('2d'); 
      return new Chart(ctx, {
        type: 'line', 
        data: {
          labels: history.ts, 
          datasets: [{ label:l, data:[], borderColor:c, backgroundColor:grad(ctx,c), fill:true, tension:0.4, pointRadius:0 }]
        }, 
        options: { 
    ...
    const cT = mkChart('chartTemp','rgb(6,182,212)','Temp');
    const cH = mkChart('chartHum','rgb(236,72,153)','Hum');
    const cM = mkChart('chartMq2','rgb(245,158,11)','Gas');
    ...
    async function up() {
      try {
        ...
        if (history.ts.length > MAX_PTS) { 
          history.ts.shift(); cT.data.datasets[0].data.shift(); cH.data.datasets[0].data.shift(); cM.data.datasets[0].data.shift(); 
        }
        
        history.ts.push(now); 
        cT.data.datasets[0].data.push(d.temperature); 
        cH.data.datasets[0].data.push(d.humidity); 
        cM.data.datasets[0].data.push(d.mq2);
        
        cT.update('none'); cH.update('none'); cM.update('none'); 
        log(`T:${d.temperature} H:${d.humidity} G:${d.mq2}`);
      } catch(e){}
    }
    
    setInterval(up, 2000); 
    fetch('/meta').then(r => r.text()).then(ip => document.getElementById('ip').innerText = ip);
  </script>
</body>
</html>
)rawliteral";
...
void handleSetThreshold() {
  if (server.hasArg("value")) {
    gasThreshold = server.arg("value").toInt();
  }
...
void loop() {
  ...
  if (millis() - lastSensorMillis >= sensorInterval) {
    ...
    if (analogRead(MQ2_PIN) > gasThreshold) {
